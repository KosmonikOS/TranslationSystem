...
public abstract class CommandHandler
{
    ...
    {
        return command.IndexOf(' ') > 0 ? command.Substring(command.IndexOf(' ') + 1).Split() : Array.Empty<string>();
    }
    ...
    private void TryAssignProperties<T>(Message message, PropertyInfo[] properties, T dto)
    {
        ...
        {
            if (map.Attribute is null || arguments.Length < map.Attribute.Position) continue;
